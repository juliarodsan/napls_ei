%% Fit DCM to single subject MMN data

function napls2_mmn_fit_dcm_to_individuals(subj)

%% Options
version            = 14; % Set version (will be appended to results directory and DCM file)
...
switch user_id
    case 'ahjr-radams-lt\julia' % Julia's paths
        pmain = 'D:/NAPLS2_FASTER/MMN_preprocessed_Hamilton/polished/';
        pcode = 'C:/Users/Julia/Documents/NAPLS/napls_ei/code/';
    otherwise % Julia's cluster paths
        pmain = '/SAN/intelsys/Psycho_Pheno2/Data/NAPLS2_FASTER/MMN_preprocessed_Hamilton/';
        pcode = '/SAN/intelsys/Psycho_Pheno/Dropbox/Rick/Academic/Rodriguez-Sanchez/Code/NAPLS/dcm_ei_sim/code/';
end

pdata = fullfile(pmain,'subjects');
presults = fullfile(pmain,'dcm_ei',['mmn_all_subjects_v' num2str(version)]);
fep = fullfile(pmain,'dcm_ei','priors','napls_dcm_mmn_grandmean_all_cmc_ei_v1_vbsv898_ei.mat');

disp(['Results will be saved in ' presults]);

%% Settings
% Set file prefix
dcm_prefix = 'dcm_mmn'; % prefix of DCM file (model, ID and version will be appended)

% Condition names
conditions = {'standard', 'double deviant'};

%% Get data files
files = dir(fullfile(pdata,'Mp*spm.mat'));
disp(['There are ' num2str(numel(files)) ' data files'])
    
% Get ID
id = extractBetween(files(subj).name,'NAPLS-','-1'); id = id{:};
