...
function napls2_average_mmn(opt)

%% Grand mean across all subjects
outfile = fullfile(opt.pmean, 'mmn_grandmean_all.mat');

if isfile(outfile)
    disp('Grand mean file already exists:');
    disp(outfile);
else
    %% Load subject data
    ...
    S.outfile = fullfile(opt.pmean, 'mmn_grandmean_all.mat');
