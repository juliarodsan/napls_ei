function [lineOut, fillOut] = stdshade(F,amatrix,alpha,acolor,style,smth)
% usage: stdshading(F,amatrix,alpha,acolor,smth)
% plot mean and sem/std coming from a matrix of data, at which each row is an
% observation. sem/std is shown as shading.
% - F assignes the used x axis (default is steps of 1)
% - amatrix is the data matrix (e.g. subjects x time)
% - alpha defines transparency of the shading (default is no shading and black mean line)
% - acolor defines the used color (default is red) 
% - smth defines the smoothing factor (default is no smooth)
% smusall 2010/4/23
% modified by jrs 2026
...
hold on;
if exist('style','var')==0 || isempty(style)
    lineOut = plot(F,amean,'color',acolor,'linewidth',1.5); % change color or linewidth to adjust mean line
else
    lineOut = plot(F,amean,'color',acolor,'linestyle',style,'linewidth',1.5); 
end
