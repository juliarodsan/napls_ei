function [GCM,gen] = spm_dcm_simulate_DH(GCM, mode, noise, gen_idx)
...
switch model
    ...
    case {'CSD'}
        [GCM, gen] = simulate_csd(GCM, mode, noise, gen_idx);
    otherwise
...
%--------------------------------------------------------------------------
function [GCM, gen] = simulate_csd(GCM, mode, noise, gen_idx)
% Simulate determinstic DCM for ERP
...
for i = 1:ns
    ...
    %----------------------------------------------------------------------   
    Hc  = spm_csd_mtf(DCM.Ep,DCM.M,DCM.xU);                  % prediction
    ...
    M.l         = DCM.M.m;
    qp          = DCM.Ep;
    qp.L        = ones(1,DCM.M.m);        % set virtual electrode gain to unity
    ...
    % Generate noise (in mode space)
    %--------------------------------------------------------------------------
    warning('Noise for simulation not implemented yet. Will disregard noise specification.')
    % for c = 1:length(Hc)
    %     y{c} = Hc{c};
    % 
    %     real_part = randn(size(y{c}));
    %     img_part = randn(size(y{c}));
    % 
    %     e    = complex(real_part, img_part);
    % 
    %     switch lower(mode)
    %         case 'snr_std'
    %             e    = spm_conv(e,8,0);
    ...
    %----------------------------------------------------------------------
    for j = 1:nm
        GCM{i,j}.Hc = Hc;                   % conditional responses (y), channel space
        % GCM{i,j}.Rc = Ec;                   % conditional residuals (y), channel space
        GCM{i,j}.Hs = Hs;                   % conditional responses (y), source space
    end
